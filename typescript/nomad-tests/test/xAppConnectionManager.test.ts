...
describe('XAppConnectionManager', async () => {
  ...
  before(async () => {
    ...
    // deploy nomad on remote domain
    // NB: as tests stand currently, this only needs to be done once
    await deploys.deployNomad(remoteDeploy);
  });

  beforeEach(async () => {
    // deploy nomad on local domain
    await deploys.deployNomad(localDeploy);
  ...
  it('Unenrolls a replica given valid SignedFailureNotification', async () => {
    ...
    const { failureNotification, signature } =
      await nomad.signedFailureNotification(
        watcher,
        remoteDomain,
