...
import { Updater, NomadState, MessageStatus } from 'lib/core';
...
import * as contracts from '@nomad-xyz/contracts/dist/core';
...
describe('Replica', async () => {
  ...
  before(async () => {
    [signer, fakeSigner, nomadMessageSender] = await ethers.getSigners();
  ...
  it('Halts on fail', async () => {
    ...
    expect(await replica.state()).to.equal(NomadState.FAILED);
  ...
  it('Processes a proved message', async () => {
    const sender = nomadMessageSender;
    ...
    await replica.setMessagePending(nomadMessage);

    // Ensure proper static call return value
    const success = await replica.callStatic.process(nomadMessage);
    expect(success).to.be.true;

    const processTx = replica.process(nomadMessage);
    await expect(processTx)
      .to.emit(replica, 'Process')
      .withArgs(nomad.messageHash(nomadMessage), true, '0x');
  });
  ...
  it('Fails to process an unproved message', async () => {
    ...
    await expect(replica.process(nomadMessage)).to.be.revertedWith('!proven');
  });
  ...
  for (let i = 0; i < badRecipientFactories.length; i++) {
    it(`Processes a message from a badly implemented recipient (${
      ...
      // Set message status to MessageStatus.Pending
      await replica.setMessagePending(nomadMessage);
      await replica.process(nomadMessage);
    });
  }
  ...
  it('Fails to process message with wrong destination Domain', async () => {
    ...
    await expect(replica.process(nomadMessage)).to.be.revertedWith(
      '!destination',
    );
  });
  ...
  it('Processes message sent to a non-existent contract address', async () => {
    ...
    // Set message status to MessageStatus.Pending
    await replica.setMessagePending(nomadMessage);
    await expect(replica.process(nomadMessage)).to.not.be.reverted;
  });
  ...
  it('Fails to process an undergased transaction', async () => {
    ...
    await expect(
      replica.process(nomadMessage, { gasLimit: 500000 }),
  ...
  it('Returns false when processing message for bad handler function', async () => {
    ...
    let success = await replica.callStatic.process(nomadMessage);
  ...
  it('Proves and processes a message', async () => {
    ...
    const messageHash = nomad.messageHash(nomadMessage);
    ...
    await replica.proveAndProcess(nomadMessage, path as BytesArray, index);
  ...
  it('Has proveAndProcess fail if prove fails', async () => {
    ...
    await expect(
      replica.proveAndProcess(nomadMessage, path as BytesArray, index),
