...
function serializeCall(call: types.CallData): string {
  const { to, data } = call;
...
function serializeCalls(batch: types.CallData[]): string {
...
function formatBatch(callsData: types.CallData[]): string {
  return ethers.utils.solidityPack(
    ['bytes1', 'bytes32'],
    [GovernanceMessage.BATCH, batchHash(callsData)],
  );
}

export const nomad: types.HardhatNomadHelpers = {
  formatMessage,
  governance: {
    serializeCall,
    serializeCalls,
