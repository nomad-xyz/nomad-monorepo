...
import { bridge, core } from '@nomad-xyz/contracts';
...
export class NomadContext extends MultiProvider {
  ...
  async resolveRepresentation(
    ...
    const address = await bridgeContracts?.bridgeRouter[
    ...
    let contract = new bridge.BridgeToken__factory().attach(evmId(address));
