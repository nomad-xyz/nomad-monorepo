...
export class BridgeContracts extends Contracts {
  domain: number;
  bridgeRouter: bridge.BridgeRouter;
  ethHelper?: bridge.ETHHelper;
  ...
  constructor(
    ...
    this.domain = domain;
    this.bridgeRouter = new bridge.BridgeRouter__factory(signer).attach(
      bridgeRouter,
    );
    if (ethHelper) {
      this.ethHelper = new bridge.ETHHelper__factory(signer).attach(ethHelper);
    }
  }
