...
impl<S> Home<S> {
    ...
    fn check_sig(&self, update: &SignedUpdate) -> Result<(), NomadError> {
...
impl Home<Waiting> {
    ...
    fn _update(&mut self, update: &Update) -> Result<(), NomadError> {
        if update.previous_root != self.committed_root {
            return Err(NomadError::WrongCurrentRoot {
        ...
        Err(NomadError::UnknownNewRoot(update.new_root))
    }
    ...
    /// Update the root
    pub fn update(&mut self, update: &SignedUpdate) -> Result<(), NomadError> {
