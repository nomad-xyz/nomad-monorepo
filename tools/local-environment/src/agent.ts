...
export class LocalAgent extends DockerizedActor implements Agent {
  ...
  async createContainer(): Promise<Docker.Container> {
    ...
    // docker run --name $1_$2_agent --env RUN_ENV=latest --restart=always --network="host" --env BASE_CONFIG=$1_config.json -v $(pwd)/../../rust/config:/app/config -d gcr.io/clabs-nomad/nomad-agent ./$2
