...
export class Nomad {
  ...
  getChain(network: Network): Chain {
    const deployer = this.deployers.get(network.domain);
    if (!deployer)
      throw new Error(
        `Deployer key for ${network.name}(${network.domain}) was not found`
      );

    return toChain({
      name: network.name,
      rpc: network.location.toString(),
      domain: network.domain,
      deployerKey: deployer.toString(),
      chunk: 100,
      timelag: 5,
      // gasLimit: 1,
      // gasPrice: 1,
